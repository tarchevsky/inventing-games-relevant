```shell
git clone --no-checkout https://github.com/tarchevsky/inventing-games-relevant.git temp_repo && cd temp_repo && git sparse-checkout init --cone && git sparse-checkout set dist && git checkout && cp -r dist/. ../ && cd .. && rm -rf temp_repo
```
